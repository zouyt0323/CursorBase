...
| 功能 | 说明 |
|------|------|
| **MCP Apps** | 交互式 UI 直接嵌入 Cursor：Amplitude 图表、Figma 设计稿、tldraw 白板等 |
| **Team Marketplaces** | 企业/团队版可创建内部插件市场，集中管控私有插件的分发与权限 |
| **JetBrains 集成** | 通过 Agent Client Protocol (ACP) 支持 IntelliJ IDEA、PyCharm、WebStorm 等 |
| **Debug Mode 增强** | 核心调试能力提升 |
